...
import type { Skill, SkillCategory, SkillsSectionProps } from "../types";
...
const SkillsSection: React.FC<SkillsSectionProps> = ({ skillCategories }) => {
  ...
  return (
    <section className="py-16 px-6 bg-linear-to-br from-secondary-50 via-white to-primary-50">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold gradient-text mb-4">
            Technical Skills
          </h2>
          <div className="w-32 h-1 bg-linear-to-r from-secondary-500 to-primary-500 mx-auto rounded-full"></div>
        </div>

        <div className="grid md:grid-cols-2 gap-8">
          {skillCategories.map((category: SkillCategory, catIndex) => (
            <div key={catIndex} className="group relative">
              <div className="absolute inset-0 bg-linear-to-br from-primary-200/30 to-secondary-200/30 rounded-2xl transform -rotate-1 group-hover:rotate-1 transition-transform duration-300"></div>
                    ...
                    <div key={skillIndex} className="group/item">
                      ...
                      <div className="relative">
                        <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                          <div
                            className={`h-3 rounded-full bg-linear-to-r ${getLevelColor(skill.level)} ${getLevelWidth(skill.level)} transition-all duration-1000 ease-out relative overflow-hidden`}
                          >
